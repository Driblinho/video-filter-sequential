...
Mat EdgeDetection::getEdge(cv::Mat frame)
{
    this->img = std::move(frame);


        vector<vector<double>> filter = createFilter(3, 3, 1);

        grayscaled = Mat(this->toGrayScale()); //Grayscale the image
...
Mat EdgeDetection::toGrayScale()
{
    ...
    for (int i = 0; i < img.rows; i++)
        for (int j = 0; j < img.cols; j++)
        {
            ...
            grayscaled.at<uchar>(i, j) = static_cast<uchar>(newValue);

        }
    return grayscaled;
}

vector<vector<double>> EdgeDetection::createFilter(int row, int column, double sigmaIn)
{
    ...
    float coordSum = 0;
    auto constant = static_cast<float>(2.0 * sigmaIn * sigmaIn);
...
Mat EdgeDetection::useFilter(Mat img_in, vector<vector<double>> filterIn)
{
    ...
    for (int i = size; i < img_in.rows - size; i++)
    {
        for (int j = size; j < img_in.cols - size; j++)
        {
            ...
            filteredImg.at<uchar>(i-size, j-size) = static_cast<uchar>(sum);
        }
    ...
    for (int i = size; i < gFiltered.rows - size; i++)
    {
        for (int j = size; j < gFiltered.cols - size; j++)
        {
            ...
            if(sq2 > 255) //Unsigned Char Fix
                sq2 =255;
            filteredImg.at<uchar>(i-size, j-size) = static_cast<uchar>(sq2);

            if(sumx==0) //Arctan Fix
                angles.at<float>(i-size, j-size) = 90;
            else
                angles.at<float>(i-size, j-size) = static_cast<float>(atan(sumy / sumx));
        }
    }
...
Mat EdgeDetection::threshold(Mat imgin,int low, int high)
{
    if(low > 255)
        low = 255;
    if(high > 255)
